...
* Specification: COVID Period Analysis (2020-2021)
...
use "treatment_amr_covid.dta", clear
rename amr Y
...
did2s Y, first_stage(i.i i.t X1) second_stage(F*event L*event) treatment(D) cluster(i)
matrix did2s_b = e(b)
matrix did2s_v = e(V)
...
preserve
stackedev Y X1 F*event L*event, cohort(treat_year) time(t) never_treat(no_treat) unit_fe(i) clust_unit(i)
restore
...
reghdfe Y X1 F*event L*event, absorb(i t) vce(cluster i)
matrix ols_b = e(b)
matrix ols_v = e(V)
...
, stub_lag(L#event) stub_lead(F#event) ///
  ...
  graph_opt(  ///
      title("Antimicrobial Resistance Prevalence: Event-Study Estimates (COVID Period)") ///
...
graph export amr_did_covid.svg
