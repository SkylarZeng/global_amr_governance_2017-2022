...
* Outcome: Antimicrobial Resistance Prevalence
* Specification: ±5-Year Robustness Check
...
use "treatment_amr_control.dta", clear
rename amr Y
...
gen Kc_5 = K if !missing(K)
replace Kc_5 = 5 if K > 5 & !missing(K)
replace Kc_5 = -5 if K < -5 & !missing(K)
...
forvalues l = 0/5 {
	gen L`l'event = Kc_5 == `l'
}
forvalues l = 1/5 {
	gen F`l'event = Kc_5 == -`l'
}
drop F1event
...
, stub_lag(L#event) stub_lead(F#event) ///
  ...
  graph_opt(  ///
      title("Antimicrobial Resistance Prevalence: Event-Study Estimates (±5 Years)") ///
...
graph export amr_did_pm5.svg

* --- ROBUSTNESS TEST 1: PARALLEL TRENDS ---

did2s Y, first_stage(i.i i.t) second_stage(F*event L*event) treatment(D) cluster(i)
test (F2event=0) (F3event=0) (F4event=0) (F5event=0)

* --- ROBUSTNESS TEST 2: PLACEBO TEST ---

gen Ei_placebo = Ei - 1
gen K_placebo = t - Ei_placebo
gen Kc5_placebo = K_placebo
replace Kc5_placebo = 5 if K_placebo > 5
replace Kc5_placebo = -5 if K_placebo < -5

forvalues l = 0/5 {
    gen L`l'placebo = (Kc5_placebo == `l')
}
forvalues l = 1/5 {
    gen F`l'placebo = (Kc5_placebo == -`l')
}
drop F1placebo

did2s Y, first_stage(i.i i.t) second_stage(F*placebo L*placebo) treatment(D) cluster(i)
test (F2placebo=0) (F3placebo=0) (F4placebo=0) (F5placebo=0)
