#!/usr/bin/env Rscript
# plot_Fig5B_forest.R
# Generate a publication-style forest plot for Figure 5B using model results in `output/`.
...
expected <- file.path("output", "Fig5B_fixed_model_results.csv")
if (file.exists(expected)) {
  input_file <- expected
} else {
  candidates <- list.files("output", pattern = "Fig5B.*model.*result.*\\.csv$", full.names = TRUE, ignore.case = TRUE)
  if (length(candidates) == 0) {
    candidates <- list.files("output", pattern = "Fig5B.*result.*\\.csv$", full.names = TRUE, ignore.case = TRUE)
  }
  if (length(candidates) == 0) stop("No Fig5B results CSV found in output/ (expected 'output/Fig5B_model_results.csv')")
...
forest_plot <- ggplot(df, aes(x = coefficient, y = indicator)) +
  ...
  labs(title = "Associations in countries with the greatest AMR improvements", y = NULL) +
...
out_pdf <- file.path("output", "Fig5B_forest.pdf")
dir.create("output", showWarnings = FALSE)
ggsave(out_pdf, plot = combined, width = 9.5, height = 5.5)
cat("Saved Figure 5B to:", out_pdf, "\n")
